...
**Current Status:** The NPU has been successfully integrated and tested with a Multi-Layer Perceptron (MLP) model. It correctly predicts all MNIST digits as demonstrated in the results section.
...
The system is implemented on a ZyboZ7010 FPGA board ( Xilinx Zynq-7000 series device), which combines a Processing System (PS) with ARM cores and Programmable Logic (PL).
...
*(Refer to the Python script `export_weights_colab.py` . This script should be run in an environment like Google Colab or a local setup with necessary Python libraries installed.)*
...
*(Refer to the Python script `generate_mnist_c_array_colab.py` .)*
...
*(Refer to the C code in `npu.c` for the detailed implementation.)*
...
3.  **Create Vitis Application:**
    ...
    * Create a C source file (e.g. `main.c`).
    * Copy the content from the provided Vitis C code (`npu.c`) into your source file.
